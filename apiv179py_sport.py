	...
	def categoryContent(self, cid, page, filter, ext):
		...
		for data in dataList:
			...
			for video in data.xpath(".//div[@class='list']/ul/li"):
				...
				sdate = dateList[pos].xpath('./qyg11.js')[0].split()[0].strip()
				hour = stime.split(':')[0]
				...
				if len(infosList) != 2:
					home = infosList[0].xpath('./qyg12.js')[0].strip()
					away = infosList[2].xpath('./qyg12.js')[0].strip()
					cover = infosList[0].xpath('./qyg14.js')[0]
					name = home + 'VS' + away
				else:
					cover = './qyg16.png'
					name = infosList[1].xpath('./qyg11.js')[0].strip()
				if state != '已结束':
					videos.append({
						"vod_id": rid,
						"vod_name": name,
